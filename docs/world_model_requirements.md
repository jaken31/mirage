...
F-2 and F-7 are new, and both carry real weight. F-2 (the flat render) protects
the token budget. F-7 (the occlusion floor) is what gives object permanence
(Q-6) something to score.

**The occlusion floor (F-7) was restated 2026-08-28**, after
...
**The dynamics model's acceptance test (F-11) was restated 2026-09-22**, on a
...
F-11 run that reports that baseline alongside is where the comparison gets a
number. The tier and the description are unchanged. No margin over persistence
is set: the bar is the baseline itself.

**F-11's description was amended 2026-09-23.** This is a second amendment,
...
reading. **The mask measurement is not an F-11 verdict.** It scored two probe
...
The generation throughput bar (P-6) dropped from 20k/sec. MuJoCo plus an
...
Object permanence (Q-6) is the memory result. It is back in scope because
...
Arm plausibility (Q-5) replaces velocity preservation. A model that hallucinates
arm geometry is the typical failure here.

**The coherence horizon (Q-3) was restated 2026-08-30**, after
`bench/q3_blind_probe.py` measured what its stopping check could actually see.

**The frame validator (F-9) is unchanged and is not at fault.** It is a
...
was wrong is the conclusion Q-3 drew from it: that a rollout surviving 200 F-9
checks stayed coherent.

**Implementing F-9's block-count and arm-pose halves would not have fixed this
...
**Arm plausibility (Q-5) was restated 2026-08-30**, and it took the long way
...
Q-4 treatment - a relative bar - rather than a better statistic.
...
The sanitizer requirement (E-3) was ASan **and** UBSan until 2026-08-26. MSVC has
...
| ID | Risk | Fallback |
|---|---|---|
| F-2 | The flat config may still leave gradients that hurt tokenizer PSNR (Q-1) | Drop to 96x96 / 144 tokens, promote F-16 to M |
