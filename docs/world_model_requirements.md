...
| **Q-4** | **Measured 2026-08-28 to sit above its own ceiling, and restated as a relative bar** - see its row above. Remaining risk: the ground-truth term must be recomputed whenever the scene or `action_hold_steps` changes, and an action-following row quoting only the model's number cannot be checked | Report both numbers or the row does not count. `bench/hold_probe.py` produces the ground-truth term. **Measured 2026-09-25 (Phase 2 item 6, `runs.jsonl` r61): measured the same way on a model's frames, which have no joint angles, the term is at chance.** The pixel link angle's per-step sign agrees with the command 47.1% of the time on the val split's decoded ground truth, against 89.7% for the simulator's joint angles on the same frames, so 90% of it passes any model. Phase 2's gate reports both numbers without a verdict this phase |
| **Q-3** | **Measured 2026-08-30 to stop on a check blind to dynamics failure, and its stopping check was replaced** - see its row above. Remaining risk: the continuity bound is calibrated on ground-truth frames, which are perfectly rendered, while the horizon's inputs are decoder output. That is the same two-population trap that cost build-order item 6 its obvious recipe | Calibrate on **reconstructions**, not renders. Keep `bench/q3_blind_probe.py` as the regression test: a check that stops firing on the 300-step substitution has silently gone blind again. **Measured 2026-09-25 (Phase 2 item 6, `runs.jsonl` r61): the replacement fails that test.** Bounded at the largest per-step change over the val split's decoded cached rows, so no ground truth fires, it catches 53.7% of the substitutions, not 100%, because the per-step noise of its features on decoded frames swamps the motion. Phase 2's gate reports the horizon rather than passing or failing it until this requirement is restated, which is the captain's call |
