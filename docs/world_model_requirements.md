...
| **Q-4** | **Measured 2026-08-28: the 90% bar is above its own ceiling.** Scored as `sign(theta_t+1 - theta_t)` against the commanded sign, **ground-truth frames themselves reach only 83.1%** at `action_hold_steps = 20` - for about one settling time after each sign flip the joint is still moving the old way, and joint0's settling time is 21-34 steps. A model reproducing the simulator exactly would fail this requirement by 7 points. **This is a defect in the requirement, not in any model.** `bench/hold_probe.py` | Three, and one must be chosen before Phase 2 scores anything. **(a)** Raise `action_hold_steps` to 60, where ground truth reads 91.3% - costs window coverage, 19.4% of ctx windows then carry an action change against 62.1% today. **(b)** Scene edit: scale `gear` and `damping` together, which shortens the transient at constant arm speed - measured to give 90.3% at hold 15 with 83.3% window coverage, but moves `data_hash` and needs F-5, F-6, F-7 re-verified. **(c)** Score Q-4 against the simulator's own number rather than an absolute 90% - the honest form of the metric, since the ceiling is a property of the physics and not of the model |
