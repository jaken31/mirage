...
| MuJoCo step time for this scene | **measured** - `bench/step_probe.py`, 5x1000 steps, reset every 200, idle and driven arms | **10.8 us median / 46.7 us p99 driven; 9.7 / 31.6 idle**, at 12-14 total contacts. **171x under the ~1850 us P-6 allowance - P-6 is not at risk from physics.** Two caveats: `ncon` is scene-total and dominated by three blocks resting on the table, so the idle/driven gap is not contacts-vs-no-contacts and the contact-fraction guard was vacuous; and max is 11-15x median on both arms, which is Windows scheduler preemption rather than solver work. Rerun spread 16-22% is the timer noise floor at a 10 us median, not E-4 instability. CPU-only - GPU pstate does not apply |
