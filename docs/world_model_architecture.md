...
- **The KV cache, when enabled, is allocated once at a fixed size and masked. It
  ...
  This does not mean "the cache is always on": the inference ladder needs
...
- **Sanitizers are available from the first C++ file** (`/W4 /WX /fsanitize=address`),
  ...
  to meet the generation throughput bar. MSVC has no UBSan; see the
...
The deciding argument is the sanitizer requirement (ASan clean on the full
...
That fits under the 20 GB dataset ceiling with room for the 96x96 fallback
(8.294 GB).
...
every scripted frame as a contact. That takes the contact rate from 16.6%
...
| Field | Consumer |
|---|---|
| `action` | action histogram for the policy-mix check, training targets, commanded direction for action-following |
| `qpos[2]` | calibration reference for pixel-measured link angles (action-following, arm plausibility) |
| `block_xy` | object-permanence position error |
| `visible_px[3]` | occlusion rate, object-permanence occlusion events |
| `contact_mask` bits 0..6 | contact rate |
| `contact_mask` bit 7 (scripted) | which half of the policy mix produced the episode - per-half breakdowns of contact rate, action-following and window coverage, none of which the dataset could answer before |
...
on the generation bar, and `offsamples="0"` is separately required for the
flat-render config's <=24-colour palette.
...
RGB from a palette lookup.** Under the flat-render config - ambient-only light, no shadows,
...
the segmentation frame. The benefits: the <=24-colour ceiling holds by
construction instead of by test, determinism gets stronger because no float
...
The scene XML must be inside `data_hash`, or bench reproducibility has a
...
`(dynamics_hash, validator_hash)`, so recomputing the coherence horizon
...
destroys the very property the 50/50 mix exists for. The near-uniform action
...
at the policy-mix check's own sample size, 2,000 episodes x 600 steps: minimum share **7.15%**,
...
diagnostic, not a requirement. The contact rate is the number that would
...
class that action-following's balanced eval subset needs. **The link from
examples-per-action to action-following accuracy is unverified**: no inverse
dynamics model exists yet. Trigger to raise the floor: action-following misses 90% and its
...
No `--replay` mode. Determinism is tested by generating twice with the same
...
| `link_extent[2]` | arm link | link-length drift |
| `link_angle[2]` | arm link | **action-following** |
| `offpalette_px` | frame | palette violation |
| `n_unique_colors` | frame | the renderer's <=24-colour bar, mode 1 only |

This buys three things:

- **The coherence horizon can be recomputed without re-running rollouts.**
...
- **Action-following needs no inverse dynamics model (IDM).** Actions are
  ...
  **Run the same measurement on ground-truth frames and report both numbers.**
  ...
  `action_hold_steps`. **So action-following is scored as a fraction of that ceiling, not
...
not effort. The occlusion bar requires full occlusion in >=3% of frames, so partial occlusion is
...
so it must be computed first to serve the <=24-colour bar. And `offpalette_px` means "distance to
...
The <=24-colour bar is a statement about the renderer. An FSQ decoder emits hundreds of
...
threshold added. So each extra threshold makes the frame validator's "zero false positives"
strictly harder, and `offpalette_px` already responds to the same colour-mush
failure.

### Two modes, split by whether ground truth exists

This is what makes the frame validator's acceptance test something you can run rather than a
judgement call.
...
Its "zero false positives on ground-truth frames" is literally a threshold sweep
...
| Layer | What | Status |
|---|---|---|
| 1 | Human-authored run log: config hash, change, number, conclusion | **Must.** A lab notebook, one line per run. No tool produces it. Lives at `runs.jsonl`; one JSON object per line, keys `date`, `run`, `hash`, `requirement`, `change`, `number`, `conclusion`. `hash` is null for a run that reads no config - the `bench/` probes do not. `number` is an object rather than prose so the bench's jsonl-to-markdown script can read it. Backfilled 2026-08-28 from the dated, numbered rows of the verification log below. |
...
account. The bench's ladder table is a jsonl-to-markdown script; no UI
produces it.

Three rules, each tied to a Must-tier number:

- **Nothing logs inside the Phase 4 timed region.** The p99 frame-time bar
  (<= 40 ms) and the p99/p50 ratio (<= 1.3) are tail-latency requirements. An
...
- **The training and inference VRAM bars use `torch.cuda.max_memory_allocated()`, not W&B's GPU
...
transform at load time, and a module swap). So the ladder's five flags
...
- **Inference needs a seed clip, so the dataset outlives the simulator.** The
  control loop drives the model with MuJoCo absent, but the model needs ~15
  ...
  10-second cold-start bar includes loading that clip.
...
> always the expensive half: DiagD from reserve to required, the fused Triton block to **M**, and
...
| Inference VRAM vs the 2 GB bar | non-issue | non-issue |
| Dataset on disk vs the 20 GB ceiling | 3.686 GB | 8.294 GB |
...
Against the 7.5 GB training VRAM bar, the 144 path caps batch at ~16 with materialized attention
...
**Therefore the fallback only helps if the reconstruction bar fails on edge placement.** If the failure
...
below - DiagD required, the fused Triton block promoted to **M**, graphs as table stakes rather than the
...
existed. The two bars are the 30 dB held-out PSNR bar and the 70% token-entropy bar.

**PSNR.** A tokenizer that codes each patch independently cannot pass.
...
**Entropy.** The data does not force low entropy. Only **20.28%** of interior
...
**The direct evidence for a collapse is gone, and the entropy bar is now an open
question rather than a prediction.** The 150-of-512 live-centroid figure that made
it a prediction came from the unrecorded random init again; k-means++ keeps **486
of 512** alive on held-out patches. The entropy bar is about a *trained
tokenizer's* code usage, not about k-means, so neither figure settles it. What
changed is that nothing now argues for shrinking the vocabulary in advance.

**If the entropy bar misses, shrink the vocabulary. Do not add an entropy loss.**
An auxiliary loss undoes the reason FSQ was chosen over VQ, and it makes the
collapse question permanently unanswerable. The ladder, in order: `[8,8,8]`=512 ->
`[8,6,5]`=240 -> `[5,5,5]`=125 -> `[4,4,4]`=64. The token count never changes, so
inference cost is untouched and the output head shrinks.
...
counterweight is recalibrating the frame validator on reconstructions:
...
- **R2's encoder attention makes tokens depend on batch size.**
  ...
  determinism risk that R1 structurally cannot have.
...
costs nothing against the 7.5 GB training VRAM bar. It also removes a class
...
lookup-table size. It stays a `uint8` index up to 256 entries, well past the flat
render's 24-colour ceiling.
...
The sanitizer requirement asks for ASan clean on the **full** data-generation
...
**Decided 2026-08-26: the sanitizer requirement drops the UBSan half rather than
adding a clang-cl configuration.** Of the UBSan classes that apply to this code -
MuJoCo calls, a pixel buffer, segid integer math, appending to a blob - ASan
already covers out-of-bounds access and use-after-free, and `/W4 /WX` rejects the
sloppy casts (`/w14242` lossy conversion, `/w14826` sign extension). The one real gap is
...
**On Linux the UBSan half exists, and the sanitizer build uses it** (added
2026-09-23, `runs.jsonl` r55). `MIRAGE_ASAN` there is Clang with
`-fsanitize=address,undefined -fno-sanitize-recover=undefined`, so a UBSan report
fails the run rather than printing and exiting 0. Clang only: MuJoCo 3.12.0's
`mjsan.h` switches on under ASan and puts an attribute after a function
definition's declarator, which GCC 16.2.1 rejects as a hard error. The full
300,000-frame run under it has no ASan or UBSan report. The decision above still stands for the
Windows build, where none of this is available.
...
- **No leak checking on MSVC.** On Linux LeakSanitizer does run, and every leak
  it has found is in a library: ~1.7 KB per run from the NVIDIA driver (inside
  `libdbus`, plus a driver module already unloaded when the report is made, so a
  suppression file cannot name it), and ~140 KB per keyboard-keymap event from
...
1. **Bench reproducibility ("rerun matches within 5%") cannot be met without
...
2. **The p99/p50 ratio bar (<= 1.3) can be blown by a single boost transition
   mid-run**, whatever the kernels do. On a laptop with dynamic boost this is the
   most likely cause of failing that bar, and it would look exactly like a kernel
   problem.
3. **So the bench harness must record `pstate`, `clocks.current.sm`,
   ...
   it is not a valid bench number. This is cheap: one `nvidia-smi` query per run,
...
- **At 64 tokens, the 30 fps bar needs graph capture and nothing beyond it;
  after that, the 3x speedup bar is the one that binds.** **Corrected
...
- **A prediction to check the instrumentation:** the bench's first CPU/GPU split
  at 64 tokens should show roughly 80% CPU dispatch, 20% GPU busy. A very
...
- **Colour the two arm links differently.** Link-length drift, `link_angle`
  for action following, and `compactness[link]` then each become a few lines of
  pixel measurement per link. A shared arm colour would need one blob segmented into two
  links.
...
- **The palette has exactly one home: the XML's `rgba` attributes**, plus one
  ...
  **Measured correction 2026-08-27: the palette is the six `rgba` colours plus
  ...
  ~578 px on a flawless frame, and the frame validator can never reach zero false
  ...
  300k frames to fix a reader's bookkeeping. Also **keep the palette unrounded**. Comparing against
...
- Keep the palette under the flat render's 24-colour ceiling. The current count is
  ~7 (background, table, 2 arm links, 3 blocks), so there is headroom, but every new
...
mirage/
  ...
  runs.jsonl                 # lab notebook, append-only, hand-authored
```
...
| **The simulator builds on Linux and regenerates the canonical dataset byte-identical** | **run 2026-09-23**, `runs.jsonl` r55. GCC 16.2.1 Release build from an empty directory, then `mirage_sim mirage/configs/base.json` rendering on the NVIDIA GPU through PRIME offload, into a scratch repo root so the Windows-generated `data/shards` stayed untouched. `bench/shard_diff.py` against that set - SHA256 of all 14 `.pixels`/`.meta` blobs - and the committed fixture the same way | **All 14 blobs identical, and the fixture's two.** 300,000 frames at 10,284 frames/s, `data_hash` `NUM-DATA-HASH64` unchanged. Repeated under the Clang ASan+UBSan build: identical again, no report, 9,258 frames/s. So OS and compiler did not change a byte. **This does not show the NVIDIA driver version is irrelevant:** Linux ran 610.57.04, the driver behind the Windows set was never recorded (r23), and the two may have been the same. Shards now record it in their sidecar's `gl_version` (r56). See the next row for what did change the bytes | `CLAUDE.md`, "Environment facts" - corrected, it said everything runs on Windows. `README.md`, "Environment" - corrected, the OS row said native Windows only. `README.md`, "Determinism caveat (F-4)" - still standing, now with the cross-platform measurement |
| **The hardware check is enough to get the GPU the dataset was made on** | **refuted 2026-09-23**, `runs.jsonl` r55. On this hybrid laptop, GLFW under Wayland picks the Intel iGPU by default. Ran the full 300,000 frames there and compared them with the Windows set by `bench/shard_diff.py` | **442 of 300,000 frames differ, each in exactly one pixel**, and 71 records' `visible_px` differ by 1; actions, joints, block positions and contacts are identical. It passes the deny-list, since it is real hardware, runs at 1,055 frames/s, and stamps the same `data_hash` on a different dataset. Nothing in the binary or sidecar records which GPU rendered a shard | `README.md`, "Build", Linux - the offload commands. `CLAUDE.md`, "Environment facts", Linux. The provenance gap itself has no fix here; it is reported as a follow-up |
| **GL_RENDERER deny-list catches software GL on Linux** | **refuted, then fixed 2026-09-23** - `LIBGL_ALWAYS_SOFTWARE=1` before and after adding `llvmpipe`/`softpipe` to `sim/gl_context.cpp` | Before: `llvmpipe (LLVM 22.1.8, 256 bits)` passed and the fixture ran at 336 frames/s. After: `software GL, not hardware: matched 'llvmpipe'`, exit 1 | This doc, "Verification", GL context row - corrected. `CLAUDE.md`, "Hardware check on Windows" - still standing, Windows-only by its heading |
| **A Linux sanitizer run would need an LSan suppression file against the NVIDIA driver** | **measured 2026-09-23** - Clang ASan build over the fixture on NVIDIA and Intel, with and without `leak:libdbus-1.so` suppressed, then the full 300,000 frames on Intel with leak checking on | **A suppression file is not enough, and the driver is not the only source.** NVIDIA fixture: 1,719 bytes in 5 allocations; the 1,495 in `libdbus` suppress, the other 224 come from an unloaded module with no name to match. Intel fixture: no report. Intel full run: **6,684,478 bytes in 932 allocations, all under GLFW's `keyboardHandleKeymap`** - 47 keymap events, and GLFW 3.5.1's `wl_window.c` overwrites `_glfw.wl.xkb.composeState` without freeing the previous one. No ASan or UBSan error on either full run | This doc, "Sanitizer cost, and keeping E-3 cheap" - corrected, the leak bullet |
| **On Linux the binary can pick the NVIDIA GPU itself, with nothing exported** | **run 2026-09-23**, `runs.jsonl` r56. `sim/gl_context.cpp` sets `__NV_PRIME_RENDER_OFFLOAD=1` and `__GLX_VENDOR_LIBRARY_NAME=nvidia` before `glfwInit` and aborts unless `GL_RENDERER` contains `NVIDIA`. Tried each variable alone and together under GLFW's Wayland and X11 backends before the change, then the full 300,000 frames with all three offload variables unset, Release and ASan+UBSan, each compared with the Windows `data/shards` by `bench/shard_diff.py`; then the same config with `__EGL_VENDOR_LIBRARY_FILENAMES` pointed at Mesa | **Confirmed, and the Intel path now refuses.** Wayland needs only the offload variable, X11 needs both, so the binary sets both. **14 of 14 blobs identical** in both builds, 16,759 and 14,968 frames/s, no sanitizer report. With Mesa's vendor file it stops at context creation - `GL_RENDERER is 'Mesa Intel(R) Graphics (ARL)', not NVIDIA` - before any shard directory exists. Side effect: `LIBGL_ALWAYS_SOFTWARE=1` alone now gets NVIDIA, so reaching `llvmpipe` to test the deny-list also needs Mesa's vendor file; it still aborts on `llvmpipe` | `README.md`, "Build", Linux - corrected, it told you to export the variables. `CLAUDE.md`, "Environment facts", Linux - corrected the same way. F-3 in the requirements doc - now also an allow-list on vendor, on Linux. The "hardware check is enough" row above - its "no fix here" is superseded |
| **Recording the renderer in the sidecar leaves `data_hash` and the canonical set alone** | **run 2026-09-23**, `runs.jsonl` r56. Sidecars gain `gl_renderer` and `gl_version`, escaped through `nlohmann/json`. `python check.py` against the canonical `data/shards`, which has neither field; `shard_writer_self_check` with a renderer name holding a quote and a backslash, then again with the escape replaced by plain quoting | **Confirmed.** `data_hash` is computed from config and never saw the sidecar; `load_shards` reads neither field, so the canonical set loads and `check.py` passes. The self-check round-trips the name, and aborts with a JSON parse error when the escape is removed. GL_VERSION reads `4.6.0 NVIDIA 610.57.04`, so it carries the driver version | This doc, "Shard format" - the sidecar field list. The "simulator builds on Linux" row above - its claim that the NVIDIA driver build did not change a byte is withdrawn, since the Windows driver was never recorded |
...
| **The row above's per-row `run_id`/hash assertion, actually exercised** | **run 2026-08-30**, `runs.jsonl` r53, same `python -m mirage.logging --network mirage`, re-run at the head that added the per-row check (`2c576c0`..`a6ded8c`) rather than the head r52 measured. Run twice; the second timed | **Both runs passed it.** The row above recorded "measured" a check that had not executed when it was written - the run_id/hash comparison was added to `_network_check` after r52's run, so r52's own claim rested on code that postdated its evidence. This row is that gap closed: run `rjeox38l`, then timed run `ednqp4f4` (**10.9 s**, project already existing, consistent with r52's 10.5 s), both passed it, 3 of 3 records, as two distinct comparisons: locally, every record read back off the jsonl on disk carries the in-process `run_id` and its one hash column - no network in that half, since `run_id` and the hashes are never sent per row and live in `wandb.init`'s config; server-side, the history fetched back through `wandb.Api().scan_history` matches every `step` and `loss` value. No code changed; this row exists only to move the assertion from added-but-unexercised to measured | None - r52's wording is left as written, since it was true at the time it was recorded; this row is the correction, not an edit to it |
| **The online login guard's exemption predicate, `Settings._offline` / `._noop`** | **run 2026-08-31**, no notebook row - a read of the installed package, not a measurement campaign. `py.exe -c` constructing `wandb.Settings(mode=m)` for all six mode literals 0.29.0 accepts (`online`, `offline`, `shared`, `disabled`, `dryrun`, `run`) and reading both attributes, plus `inspect.getsource` on each property | **The guard exempts exactly the non-networked modes.** `_offline` is True for `offline` and `dryrun` only; `_noop` is True for `disabled` only; both are False for `online`, `shared` and `run`. That is why the guard asks wandb instead of spelling out a mode tuple - an earlier tuple missed `dryrun`, whose `login` returns False on principle, and refused to start a run that never touches the server. **Both attributes are private**: an upgrade may rename them, and then every online mirror run raises `AttributeError` at construction. Not verified: that they survive any particular future wandb version | `mirage/logging.py`'s guard comment - **carries this command now** rather than asserting the semantics bare. `_bad_credentials_check` - **already refuses to report an `AttributeError`/`TypeError`/`NameError` as a credential refusal**, so the rename surfaces as a failed self-check rather than a green one |
| **`finish(exit_code=)` against the installed wandb, and a crash actually reaching it** | **run 2026-09-04**, no notebook row - a check of the installed package, not a measurement campaign. Version and interpreter read back first (`py.exe -c "import wandb, sys; print(wandb.__version__, sys.version)"` -> wandb 0.29.0, Python 3.14.2), then the offline mirror driven on that same stack with a forced `raise ValueError(...)` inside `with Run("verify-crash", ..., wandb_project="mirage-selfcheck")` under `WANDB_MODE=offline`, and the run's own `wandb/offline-run-*/logs/debug.log` read back afterwards | **The keyword exists on 0.29.0 and the nonzero code reaches the package.** `finish()` accepts an `exit_code` keyword, the exception propagated out of the `with` block as it must, and wandb's own debug log recorded `wandb_run.py:_atexit_cleanup():2512] got exitcode: 1` - so the 1 was carried inside wandb, not merely accepted by the signature. **This row is what covers the real package.** `_self_check`'s new exit-code branch asserts the contract against a `_StubWandb` double, so it pins what this module *passes* and exercises none of wandb's own behaviour; without this row that claim rests on a test double. **UNVERIFIED, and not attempted: that a crashed run reads back `state='failed'` through `wandb.Api()`.** That is the server-side half - it needs credentials, a network and an upload, all outside this change - so only the local half is measured here | Nothing retracted. `mirage/logging.py`'s `close()` comment is the site that states this contract; its `exit_code` half now has this run behind it, and the `state='failed'` read-back is not asserted anywhere |
| **Strictly-causal against block-causal attention at a fixed step budget - which mask Phase 2 trains under** | **run 2026-09-22/23**, `runs.jsonl` r54, `bench/mask_probe.py`, with `bench/gpu_probe.py` alongside and `bench/token_stability_probe.py --episodes all --first-target 15` for the baseline. Two arms of r49's `rope_untied` model (14,593,152 parameters, asserted) fed **one token stream** - decision 2's layout over a 16-frame window - and scored on the same 960 targets: strict at 1,039 positions, block at the same stream's first 975 in blocks of 65 (frame t plus `a_{t+1}`), frame t+1 read where frame t sits. One epoch at batch 16, two seeds per arm, identical batches and initial weights per seed. Scored on the **generated** next frame from ground-truth context over every val window's last frame, 1,010,880 cells, against persistence on the same cells. Rule fixed in code before the first step: the higher arm wins only past the seed spread. `--self-check` is the regression test for the layout and the mask, mutation-tested against four broken variants | **Block-causal is selected**: generated accuracy 84.39% / 84.21% against strict's 81.80% / 82.00%, a margin of 2.40 points against a seed spread of 0.20. **The mechanism is measured, not argued**: the same strict checkpoint reads 90.1% fed its target frame's true earlier cells and 81.9% fed its own, so one early wrong cell compounds through the frame, and strict keeps only ~90% of the cells that should not change against block's ~93%. **Gate row 1's measure taken literally would have picked strict** and passed a model worse than copying; under block-causal the teacher-forced and generated scores are one number by construction. **Not an F-11 verdict**: both arms sit below persistence (86.69% on this population) at a one-epoch endpoint past both arms' held-out minimum. **Unmeasured**: the generated score at each arm's held-out optimum - the recorded trigger that would reopen the choice | `world_model_requirements.md`, the Models table's F-11 description "predicts next token" - **still standing verbatim**, deliberately: the requirement prescribes a second, separately dated amendment rather than an edit, and that paragraph now sits under the table. `world_model_requirements.md`, "Requirements at risk", F-11 - **corrected**, "it has not run". `phase2_structural_plan.md`, "Before item 1", decision 9 and the decisions heading - **corrected**; "3. The model", the causality bullet, and "1. The sequence layout", the 1,040-position derivation, and the gotcha on quoting r49's 975 positions - **pointed at the result**; the proposed gate's row 1 - **corrected**. `AGENDA.md`, Phase 2's "First, before item 1" - **corrected**. `bench/dyn_size_probe.py`'s `torch.triu` mask - **still standing**, correctly: it times a matmul and was never the implementation. `handoff_tokenizer_decision.md`, open decision 3's note - **still standing**, a dated snapshot |
| **The block-causal arm's generated score at its held-out optimum is unmeasured, and its miss against persistence is overfitting past that optimum** | **refuted, run 2026-09-24**, `runs.jsonl` r59, no training. Persistence computed on the 512 val windows `bench/mask_probe.py`'s held-out curve scores (`EVAL_SUBSET_SEED` 0), which r54 did not log, and the final curve point checked against `generated.npy` on the same windows | **Under block-causal the curve's last-frame accuracy is the generated score** - equal at the final point, 27,483 of 32,768 cells both - so r54 measured the block arm's generated score at every checkpoint, without persistence beside it. Persistence on those windows is **86.47%**, and **neither block seed is above it at any of 18 checkpoints**: the best point, step 1,000, is -0.19 and -0.10 points, the held-out cross-entropy minimum, step 10,000, is -0.35 and -0.63. The accuracy peak and the loss minimum are 9,000 steps apart, so a checkpoint picked by held-out loss is not the one gate row 1 picks. **Strictly-causal's generated score at its optimum is still unmeasured**: its curve accuracy is teacher-forced | `world_model_requirements.md`, "Requirements at risk", F-11 - **corrected**. `world_model_requirements.md`, the Models table's 2026-09-23 amendment paragraph - **still standing**, true of the endpoint, with a dated paragraph added after it. `phase2_structural_plan.md`, "Before item 1" - **corrected**; "4. The training loop" - **amended**, the instrument now scores row 1's measure against persistence at sub-epoch intervals. The mask row above - **still standing**, kept as written. `runs.jsonl` r54's conclusion, "WHAT THIS ROW DOES NOT MEASURE" - **still standing**, append-only |
| **`bench/gpu_probe.py`'s compute FAIL means the GPU does not hold its clock, and the sizing probe's lower-bound timing is what schedules rest on** | **refuted, run 2026-09-24**, `runs.jsonl` r58: `python bench/gpu_probe.py` twice on Linux, each alongside `nvidia-smi --query-gpu` at 250 ms. The chosen model's timing read from r54 | **The FAIL is the probe's decay statistic.** Its 1 Hz sampler starts before the matmul, so its first third includes the idle clock (180 MHz in run 1). Run 1 fails at -5.7% decay while load samples alone give **+1.1%**, and its clock and power clauses pass. Under load the SW power cap is active on every sample at 99 W of a 100 W limit, HW thermal and power-brake counters 0, fp16 **31.6 TFLOP/s**, within 1.0% of r54's 31.3. Run 2, with the NVIDIA-driven display active, passes at -4.6% and reads 26.5 TFLOP/s. **The chosen model is already timed clocked up**: 159.6-160.1 ms/step, about 0.78 h/epoch (r54). On Linux the enforced limit reads 85 W idle and 100 W under load; r49's row records 85 W | `phase2_structural_plan.md`, "4. The training loop", the throughput paragraph and decision 4a's cap bullet, decision 4a's entry, and "The numbers already taken", the step-cost row - **corrected**. The sizing-probe row above - **still standing**, kept as written. `runs.jsonl` r54's `gpu_probe` note, "Compute FAILs on clock decay under sustained load" - **still standing**, append-only. `bench/gpu_probe.py`'s decay statistic - **still standing**, not fixed by this change. `canonical_numbers.md`, `NUM-HW-FP16` and `NUM-HW-POWER` - **corrected**, labelled Windows, with Linux siblings `NUM-HW-FP16-LINUX` and `NUM-HW-POWER-LINUX` |
| **The token cache carries 19.5 M tokens** | **corrected 2026-09-24**, a read: `runs/20260829-005439-r1/tokens/manifest.json` and `bench/dyn_size_probe.py`'s `dataset_tokens` | **The cache holds 19,200,000 frame tokens** (`"tokens": 19200000`). The 19.5 M is `frames * (per_frame + 1)`, 300,000 x 65, one action token per frame, and actions come from the meta records, not the cache. The ratio barely moves: 15.0x under Chinchilla-optimal counting actions, 15.2x on frame tokens alone | `phase2_structural_plan.md`, "4. The training loop", the overfitting paragraph, and "The numbers already taken", the data-against-capacity row - **corrected**. The sizing-probe row above - **still standing**, kept as written; its 19.5 M is right as the probe's count. `AGENDA.md`'s "15.0x under" - **still standing**, true counting actions |
| **Phase 2 item 1's working-when list: the block-causal layout, the phase assertion, and the token/action window sampler** (`mirage/dynamics.py`) | **run 2026-09-24**, no notebook row - a check of new code, not a measurement campaign. Linux, torch 2.9.1+cu130, numpy 2.4.4, against the generated set and `runs/20260829-005439-r1/tokens`. `python -m mirage.dynamics` (14.4 s), then a mutation pass: ten single-line bugs applied one at a time to a copy of the file, the self-check re-run on each | **Confirmed.** **Layout:** 15 blocks of 65 = 975 positions, 960 frame targets, equal to `bench/mask_probe.py`'s `layout("block", ...)` array for array at ctx 4, 8 and 15, and a window round-trips. **Phase assertion:** all 13,242 action changes in the windows' stream sit at `step_idx % 15 == 0`; copies shifted one record either way both fail. **Split:** 473 train / 27 val episodes, exactly `data.is_val`'s in `data.split_episodes`' order. **Windows:** all 276,705 train and 15,795 val windows are `WindowSampler`'s (episode, offset) with the same token rows and meta actions, both on an identity-coded cache whose rows name their own frame and on R1's cache (300,000 rows, manifest and sha256 match). **Cache loader** refuses each tampered manifest field, a missing row, a missing shard, a code at or past the codebook size and a flipped byte. **Mutations: 10/10 fail**, the unmutated copy passes - off-by-one tokens, off-by-one actions, offset +1, read position +1, window one frame short, action slot one back, train/val flipped, phase check disabled, sha256 check disabled, token_grid check removed | - |
| **Phase 2 item 2's working-when list: `dynamics_hash` covers every shape knob** (`mirage/config.py`, the `dynamics` section of the three configs) | **run 2026-09-24**, no notebook row - a check of new code. Linux, `python -m mirage.config`, then a mutation pass: six single-line bugs applied one at a time to a copy of `config.py`, the self-check re-run on each. `python check.py` on the generated set | **Confirmed.** An `n_heads` edit leaves `data_hash`, `tokenizer_hash` and `validator_hash` unchanged and moves `dynamics_hash` and `engine_hash`. A dropped or unknown `dynamics` key, a zero or `true` count, an undecided `mask` or `pos_encoding` value and a non-string `output_head` each raise. **Before the change both mask-measurement arms logged one `dynamics_hash`**: the strict and block checkpoints under `runs/` both carry `da1a8bf2`. It was then `4bd2ea5e` - and `87840cb9` since `rope_base` joined the section, computed by `config.load` on `base.json` - so `bench/mask_probe.py`'s evaluate and compare refuse those runs under the current config. **Mutations: 6/6 fail** - `n_heads` or `mlp_ratio` dropped from the positive-int keys, the choice check removed, its string guard removed, `dynamics` left out of the hash, an unknown `mask` value admitted. `strict_causal` is admitted on purpose, for `bench/mask_probe.py`'s strict arm, and the self-check asserts it moves `dynamics_hash`. `bench/mask_probe.py`'s `arm_config` gives the block arm `base.json`'s own hashes and the strict arm the same `data_hash` and `tokenizer_hash` with a different `dynamics_hash`, checked with torch stubbed out. **Its `train`, `eval` and `compare` on those configs are unverified**: no torch in this environment | `phase2_structural_plan.md`, item 2 and the shape-knob gotcha - **corrected**, past tense. `runs.jsonl` r49, r54 and r59's `hash` - **still standing**, append-only, the pre-item-2 value |
| **Phase 2 item 3's working-when list: the model** (`mirage/dynamics.py`) | **run 2026-09-24**, no notebook row - a check of new code, not a measurement. Linux, torch 2.9.1+cu130, CPU only. `python -m mirage.dynamics` on the fixture (17 s) and on the generated set with `runs/20260829-005439-r1/tokens` (35 s), `python check.py`, then a mutation pass: fourteen single-line bugs applied one at a time to a copy of the file, the self-check re-run on each | **Confirmed.** **Count: 14,593,152**, the sizing probe's `rope_untied` (r49) exactly, compared parameter by parameter against `bench/dyn_size_probe.py`'s model, so a mismatch prints the parameter's name; the control, its tied variant, is named as differing by `head.weight` alone. **It is the mask measurement's model**: the same `state_dict` from the same seed as `bench/mask_probe.py`'s `build_model`, the same mask, bit-identical logits. **Causality, per block**, in float64: altering positions 0, 63, 64, 65, 500, 973 and 974 leaves every earlier-block logit bit-identical and changes every position from the altered block's first on. The same assert fails under four wrong masks - none, strictly causal, blocks shifted one position, no attention across blocks. **Loss:** 15 x 64 frame targets, none read at an action slot, frame 0 context only, no target an action id, and `frame_loss` equals cross-entropy over exactly those positions. **Round trip:** one sampler window assembles to 975 positions that rebuild its 16 cache rows and actions, the rows item 1 asserted, and the model returns 960 x 512 logits for it. `build` refuses `mask` `strict_causal`, which `config.py` admits. **Mutations: 14/14 fail** - three wrong masks, a dropped `qkv` bias, a head bias, a post-norm block, RoPE off, another RoPE base, a tied head, `build` ignoring the mask knob, the loss over every position, two weakened causality checks, a vocabulary without actions. **Not measured here, on purpose:** SDPA's speed against materialized attention and RoPE's own cost, which belong to the first run | `phase2_structural_plan.md`, item 3, decision 6 and the SDPA row of "The numbers already taken" - **corrected** |
| **Phase 2 item 5's working-when list: the token-to-pixel path** (`FSQ.indices_to_codes`, `Tokenizer.decode`) | **run 2026-09-25**, no notebook row - a check of new code. Linux, torch 2.9.1+cu130, RTX 5060 Laptop, TF32 at cuDNN's default. `python -m mirage.fsq`, `python -m mirage.fsq_eval` (2.4 s) and `python check.py`; a layout probe re-encoding shards 0 and 1 of R1's cache at batch 128, TF32 on and off; then seven single-line mutations of the new code, one at a time | **Confirmed, with one plan claim refuted.** Every id of `[8,8,8]`, `[8,6,5]`, `[5,5,5]`, `[4,4,4]` round-trips both ways, **0 mismatches**, and ids decode to exactly `forward`'s codes over the whole `tanh` range. R1 loads strictly, 32 entries. On val episode 82, shard 1 frames 6016..6143 at the manifest's batch 128, decoding `reconstruct`'s own ids reproduces its uint8 frames on all 128 frames (max abs diff 0), and decoding the cached rows does on the 111 frames whose rows agree. **The other 17 differ by up to 67**, on 29 of 8,192 tokens: the cache's channels-last input re-encodes it with **0** flips over 43,200 frames, `reconstruct`'s contiguous input flips 18,523 (0.33%); with TF32 off both flip about 16,900. **Mutations: 7/7 fail** - no per-channel remainder, one digit bound for all channels, an off-by-one id range, an un-scaling off by 1e-4, the un-scaling reordered (same algebra, different rounding), channels swapped in `decode`, float ids accepted | `phase2_structural_plan.md`, item 5 ("with the batch pinned, any mismatch means the digit recovery is wrong") - **refuted**, with a new gotcha row |
| **Phase 2 item 6: the rollout and the gate** (`mirage/dynamics_eval.py`, `python -m mirage.dynamics --eval`), and the claim that a frame-to-frame continuity check on `link_angle`, `link_extent` and block bbox centres can pass `bench/q3_blind_probe.py` | **run 2026-09-25**, `runs.jsonl` r61. Linux, torch 2.9.1+cu130, RTX 5060 Laptop. `python -m mirage.dynamics_eval --calibrate-continuity`, `python bench/q3_blind_probe.py 20260829-005439-r1`, `python -m mirage.dynamics --eval 20260925-100507-dyn` with `--checkpoint` best and final, `python check.py`; two single-line mutations of `rollout` (actions read one record late; a frame written one slot early) | **The gate: confirmed. The continuity claim: refuted.** The gate prints all ten rows and exits 1 on the one-epoch run, row 1 alone failing; the rollout's first frame equals the one-pass scorer on all 27 windows, and a rerun from the checkpoint is identical. Both mutations fail the self-check. **Continuity:** bounded at the largest per-step change over 16,173 decoded val transitions (0 fire by construction), it fires on **53.7%** of 810 300-step substitutions, not 100%; no variant tried reaches 100% even with false alarms (99th-percentile bounds: 90%). **Action-following on pixels:** the per-step link-angle sign agrees with the command 47.1% on decoded ground truth, joint angles 89.7% on the same 9,480 frames | `world_model_requirements.md`, "Requirements at risk" Q-3 and Q-4 - **standing** (the requirements are unchanged; each row now carries the finding). `phase2_structural_plan.md`, gate rows 4 and 5 and the gotcha on the frame validator - corrected to reported rows |
| **Phase 2's first real dynamics run, and decision 4a's stopping rule on a real curve** (`python -m mirage.dynamics --train`, 10-epoch cap) | **run 2026-09-26**, `runs.jsonl` r62, run `20260925-235653-dyn` ([W&B](https://wandb.ai/nguyenpk31-northeastern-university/mirage/runs/lhy7a4hz)). Linux, torch 2.9.1+cu130, RTX 5060 Laptop, wandb 0.29.0. `WANDB_ENTITY=nguyenpk31-northeastern-university systemd-inhibit --what=idle:sleep python -m mirage.dynamics --train --wandb mirage` with every default: batch 16, seed 0, full 17,294-step epochs; uninterrupted | **The rule: confirmed on a real run. Gate row 1 on ten epochs' budget: not met.** Held-out cross-entropy is lowest after epoch 1 (0.4716) and rises at every epoch after it, to 1.0414 at epoch 5, while train cross-entropy falls to 0.0375. The rule tripped at epoch 3, the second consecutive rise, ran its two further epochs and stopped after epoch 5, recorded as `{'rule': 'trip', 'epoch': 5, 'trip_epoch': 3}`. 0 of 91 sub-epoch points beat persistence (86.47%) on the 512 windows; the best checkpoint, step 3,000, is -0.14 points against persistence on the full population (86.54% against 86.69%). Peak training VRAM 2.203 GB allocated at batch 16, against the 7.5 GB bar. The W&B mirror uploaded the whole run with `WANDB_API_KEY` from the environment only | - |
| **Phase 2's gate on the first 10-epoch run** (`python -m mirage.dynamics --eval`) | **run 2026-09-26**, `runs.jsonl` r63. Linux, torch 2.9.1+cu130, RTX 5060 Laptop, W&B off. `python -m mirage.dynamics --eval 20260925-235653-dyn` with `--checkpoint best` and `--checkpoint final`, then the same two on `20260925-100507-dyn` under the same code (314-356 s each) | **The gate does not pass.** Every one of the four exits 1 on rows 1 and 6; rows 2, 3, 8 and 9 pass. Row 1 on the 10-epoch run reproduces r62's full-population scores: -0.14 points (best, step 3,000) and -4.33 (final, step 86,470). Row 6: `best.pt`'s rollouts freeze on 100% of generated frames, `model.pt`'s on 44.2%, against the truth's 20.1%; the one-epoch run's read 100% and 92.0%. Re-gating the one-epoch run reproduced r61's rows 1-5 and 8-10; row 6 now fails there too, on the frozen-share clause added after r61 | - |
