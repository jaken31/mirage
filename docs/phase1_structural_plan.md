...
a second config, and three Python files - one at a time. It names the calls and
the order, and does not write the code. The design reasons are in
...
a 64x64 picture down to an 8x8 grid of whole numbers, and a *decoder* rebuilds
the picture from that grid. *FSQ* (finite scalar quantization) is how the
squeezing is made discrete. The encoder emits 3 real numbers per grid cell, each
is rounded to one of 8 fixed levels, and 8x8x8 = 512 gives the vocabulary. There
is no learned dictionary, and that is the whole reason it was chosen: a
dictionary can collapse onto a few entries, and FSQ has none to collapse.
Rounding has zero gradient everywhere, so the backward pass uses a
*straight-through estimator*: it pretends the rounding was not there. *PSNR* is
reconstruction quality, `10*log10(255^2 / MSE)`; higher is better. A cell's
*receptive field* is how many input pixels it can see. The *token cache* is the
whole dataset re-expressed as tokens instead of pixels, and it is what Phase 2
trains on.
...
`sha256(canon(sim) + canon(data) + xml_bytes)`. Raising `offwidth` from 64 to 96
in the file would change the hash of the 64x64 dataset too, and invalidate
300,000 frames just to make a *second* dataset possible. Setting it from config
leaves the XML byte-identical, and both resolutions become a config-only change.

**Making the change weakened one existing check.** `gl_context.cpp` compares
`mjr_maxViewport` against `model->vis.global.offwidth`, and `main.cpp`
cross-checks the viewport against `cfg.width`. Those are now the same number, so
what used to be two independent checks is one fact checked twice. Both are kept,
because they cost nothing and still catch a driver that hands back a viewport
other than the one asked for. The comment in `main.cpp` now says plainly that it
is a duplicate. Do not read the pair as two confirmations.
...
every `.pixels` and `.meta` blob is **byte-identical** to what is on disk now, and
`load_shards(dir, cfg.data_hash)` accepts them. That comparison is the whole
reason this item goes first: it proves the XML stayed frozen.

**Done 2026-08-28.** All 14 blobs byte-identical, `git_sha` the only sidecar
field that moved, and the fixture reproduced under the ASan build too. The full
regeneration ran at 4,980 fps (60.2 s for the set). Budget from that, not from
the 45-50 s recorded earlier the same day.

### 2. `mirage/configs/base96.json` - the second dataset

Copy `base.json` and change three values: `sim.height` 96, `sim.width` 96,
`data.shard_dir` `data/shards96`. Nothing else - specifically not
`tokenizer.stride`. `Shapes.token_grid` is `(height // stride, width // stride)`,
so stride 8 at 96x96 already gives the 12x12 = 144 token grid.

`frames_per_shard` stays 50,000. It counts **frames, not bytes**, so the shard
split is the same at both resolutions: **7 shards**, three of 43,200 frames and
four of 42,600, because episodes are spread evenly over the shards rather than
packed to the 50,000 ceiling. The largest 96x96 shard is 1.19 GB. Do not derive
...
stepping and fixed per-frame cost, not by pixel throughput. Do not size a
resolution change by pixel count.
...
self-checks now take an optional config path, so this is
...
One function. It returns a `(n, h, w)` `uint8` array of palette indices plus the
...
so one byte per pixel is lossless, and the train split takes 1.162 GB instead of
3.49 GB raw. That matters because free RAM on this machine is of the same order,
and the loader reads 6,804 frames/s cold against 109,682 warm. Training needs
~13,000, so the cold case is a real failure, and the page cache cannot be relied
on at a 3.5 GB working set.
...
2. **Assert losslessness, do not assume it.** Map each pixel to its nearest
   ...
   distance any pixel sits from its assigned entry is under 1.0. The measured
   worst is 0.75. Exact RGB equality does not work here; the reason, recorded in
   the architecture doc, is that `rgba * 255` does not land on integers.
...
the train array is 1.162 GB, and the same call at 96x96 returns 2.616 GB with the
same 7-entry LUT.

**Done 2026-08-28**, all of it, with every figure landing exactly as predicted:
1.16 GB against 3.49 raw at 64x64, 2.62 against 7.85 at 96x96, and one LUT of
seven entries shared by both. `preload` takes the palette rgb as a parameter, not
from `load_palette`, because `validator` imports this module.

`python -m mirage.data` runs the round-trip check on the **val** split only.
Building 1.16 GB (2.62 at 96x96) on every self-check run would buy no coverage
the val pass does not already give - it is the same code over 17x the frames -
and the size is arithmetic. The full build is measured once and lives in
`runs.jsonl`: 37.5 s at 64x64 and 87.8 s at 96x96, about 90 MB/s both times, so
it is limited by packing rather than by disk. That is one-time against a 22 min
run, but the 96x96 ladder pays it per rung.
...
**Done 2026-08-28**, both, plus `meta.json` per run directory.
`python -m mirage.logging` is the check. **The W&B mirror is verified offline, on
the credential-failure paths, and on a real networked run read back off the
server** - see the verification log at the end of
`docs/world_model_architecture.md` for what each row measured.

The self-check caught two bugs, both the quiet kind and worth knowing. `log()`
returned the caller's dict rather than the serialised line, so numpy fields made
the return value and the file disagree. And `Path` through `str()` writes
backslashes on Windows. It now returns `json.loads` of the written line, and uses
`as_posix()`.
...
The whole phase in one file, built in five stages. Roughly 360 lines, inside the
comfortable 400-line band. If it passes 500, the eval half should become
`fsq_eval.py`, and not before.
...
gives exactly `prod(levels)` distinct values per dimension, and
`codes_to_indices` maps one-to-one onto `0..prod(levels)-1`. No commitment loss,
...
- **Nearest-upsample plus 3x3 conv in the decoder, never `ConvTranspose2d`.**
  Transposed-conv checkerboarding shows up as misplaced edges, and misplaced
  edges are exactly the signal that decides the 64-vs-144 fork. A checkerboard
...
- **One self-attention layer on the 8x8 grid.** 64 positions, so the attention
  matrix is 64x64 and costs nothing. It is also the only mechanism through which
  the 64 codes describe the frame *jointly* rather than independently - and the
  independent case is measured: a k-means codebook of 512 entries over real 8x8
  patches, fit on the train episodes and scored on the val ones, reaches
  28.27 dB against the 30.0 dB bar. The +1.73 dB gap is what context has to buy.

  **The prediction that followed - that the attention layer's value shows up in
  gate row 2 and not row 1 - was refuted on 2026-08-29, twice over.** It could not
  show up in row 2 either, because the eval measures row 2 against the recorded
  floor rather than refitting it, which makes row 2 just row 1 minus a constant.
  And attention buys only +0.087 dB at convergence for 263,680 parameters, about
  a sixth of this file's own "within ~0.5 dB means tied" threshold. Where it does
  show up is **row 3**: +3.5 pp of token entropy, by decorrelating the three FSQ
  digits. The reasoning about *why* it was worth trying stands; the predicted
  place to look for the effect did not. The "training run's noise" clause is also
  still **unmeasured** - no seed has been repeated.
...
Scale the input to `[0, 1]`. The final conv is linear, with **no** output
activation and **no** clamp inside the loss. `tanh` on the output would saturate
on exactly the values this scene is made of - pure black void and saturated
blocks - and clamping inside the loss removes the gradient that penalises
overshoot. Clamp only when producing `uint8`.
...
entries is 47,814, so classification needs ~99.6% pixel accuracy to clear 30 dB,
while regression can hedge with a blend.
...
reconstructions punishes exactly the blur that PSNR rewards, and the two numbers
cannot both be gamed. Report them together, or neither means much.

**fp32, not bf16.** The ingredients doc's bf16 line is about the 15M-parameter
...
is verified. They are here so the first run is reproducible, and they are
expected to move:
...
| epochs, the winner | 60 | **60 x 87.6 s per epoch**, not the ~25 min written here. The run whose PSNR is quoted |
...
Then R1 through R3 again on the 96x96 dataset, which turns the fork into a
...
**0.668** for `[4,4,4]`. The `tanh` derivative is inside it, because the
straight-through estimator skips only the rounding. So a levels change rescales
the bottleneck's effective learning rate by up to 1.5x, and a comparison run at
one LR reports a levels result that is partly an LR result. Run each levels
variant at the base LR *and* at the base LR scaled by `0.858 / g_new`.
...
touching data - `prod(levels)` distinct values per dimension, index mapping
one-to-one, gradient finite - and `python -m mirage.fsq --eval` prints the gate
table.
...
**reconstructed** frames with the **original** metas, because the ground truth
came from the simulator and is still true. What changes is the expected size of
the count. `offpalette_px` reads 0 px on every *ground-truth* frame at tau (8.0
before this item, 32.0 now), but an FSQ decoder emits continuous colour, so it
will not read zero here. The slack being spent is tau over the worst distance any
real pixel sits from its palette entry (0.75 RGB units): 11x at tau 8.0, 43x at
32.0.

**When the sweep lands, write the result in this order.** First the four register
rows it moves: `NUM-VAL-TAU`, `NUM-VAL-FALSEPOS`, `NUM-VAL-HEADROOM` and
...
`configs/base.json`, which is what actually changes `validator_hash`. Last,
...
All taken from the 300,000 frames on disk, before any of Phase 1 existed.
Re-measured 2026-08-28 at `data_hash 18a76531` by `bench/patch_probe.py`, which
is also where the k-means rows' method lives. **Quote a k-means number from
anywhere else and you are probably quoting a random-init run**, which reads
2.6 dB low.
...
The PSNR bar in physical terms, since a wrong pixel costs 47,814 of squared error
on this palette: **30 dB is about 17 of a frame's 4,096 pixels completely wrong**,
the 35 dB stretch bar is 5.3, and the k-means floor is 38.4. Phase 1's task is to
halve the count a dumb dictionary produces.
