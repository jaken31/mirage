...
nothing else, so **`n_heads` was in no hash at all**. It lives as `N_HEADS = 6` in
`bench/dyn_size_probe.py`, whose own comment says "not in config". The same was
...
holds only the decided value. Admitting an alternative is a `config.py` edit,
made when its decision's trigger fires. The move changed `dynamics_hash`, so the
sizing probe's and the mask measurement's `runs.jsonl` rows carry the hash from
before it.
