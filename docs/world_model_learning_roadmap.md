...
**Rule: just-in-time, not up front.** Each track is placed a week before you need it. Only Tier 0 happens before you start.
...
Required, and nothing else replaces it. Your dynamics model is a GPT that eats image tokens instead of text tokens. If you do not understand attention, causal masking, and next-token prediction at the code level, every later decision is guesswork. Two hours of video; plan a full day to code along.

**Karpathy, "Neural Networks: Zero to Hero"** (course home: https://karpathy.ai/zero-to-hero.html)
Watch lectures 1 through 6 only if backprop, activations, or BatchNorm feel shaky. Skip them if you are comfortable. Skip the tokenizer lecture entirely: BPE is irrelevant here.
...
Read VQ first even though you are building FSQ. FSQ only makes sense as "the thing that removes VQ's problems", and what it removes is the commitment loss and the codebook-collapse machinery.
...
**The Dreamer line moved off the skip list.** DreamerV1-V3, RSSM, PlaNet, DayDreamer. In a robotics framing you will be asked about this lineage, and Dreamer is what everyone in robot learning means by "world model". Read the DreamerV3 paper once for vocabulary. Do **not** implement any of it. It is the RL branch - latent imagination for policy learning - and you are building a simulator, not an agent. Know it, skip it.

A framing worth holding onto: diffusion video models predict better than you will, and are far too slow to close a control loop. Autoregressive plus KV cache is the choice latency forces. Your whole project argues for that sentence.
...
- **`record.cc` sample** - in the MuJoCo GitHub repo under `sample/`, **not** in the pip wheel (checked: `mujoco` 3.12.0 ships headers and two test XMLs only). It does exactly what your harness does: steps physics, renders offscreen, dumps pixels. Read it before writing yours.

**Also:** build with `/W4 /WX /fsanitize=address` from the very first file (MSVC has no UBSan). While your instincts are not yet C++ instincts, sanitizers are what keep C++ tolerable instead of a nightmare.

**Note:** you are not writing a physics engine. You are writing a data pipeline against a C API. That is more useful, and closer to real infrastructure work, than hand-rolling collision detection.

---

## Track D - GPU programming (before Phase 4, ~20 hours)

This is the big one, and the order matters. **Pull steps 1 and 2 into Phase 2 background time.** Phase 4 is a sudden jump in difficulty, and this is how you make it smaller.
...
Read this before the code. It explains *why* each step exists, including one gotcha you would otherwise hit blind: a KV cache that grows breaks CUDA graph capture, so you must allocate the maximum size up front and mask the unused part. That constraint should be in your engine from the first commit.
